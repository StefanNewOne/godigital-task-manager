...
**TD-8 — Доделување при враќање може да остане без извршител.** `E_ASSIGN(mon)`/`E_ASSIGN(diz)` при враќање (`vnatresno→montaza`, `vnatresno→dizajn`) немаат `G_ASSIGNEE_REQUIRED`, па ако нема `payload.assigneeId` ниту `client.defaultAssignees[role]`, `resolveAssignee` враќа `null` и таскот се одбележува недоделен — own-scope улогата (mon/diz) нема да го види. Причина: оригиналниот монтажер/дизајнер не се чува одделно (нема `monId`/`dizId` како `rezId`/`kreaId`). Поправка: при враќање задржи го претходниот извршител (нов `previousAssigneeId` или посебни колони), или барај `assigneeId` во payload при враќање. Ризик: среден (влијае на видливост при враќање). Пронајдено при анализа на state machine (2026-09-24).
